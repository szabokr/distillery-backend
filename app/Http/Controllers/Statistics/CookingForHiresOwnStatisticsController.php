...
class CookingForHiresOwnStatisticsController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        if ($request->has('year') && $request->has('month')) {
            $cookingForHireStatistics = Cooking_for_hire_statistic::where('own', 1)->whereYear('created_at', $query['year'])->whereMonth('created_at', $query['month'])->get()[0];
        ...
        $cookingForHireStatistics = Cooking_for_hire_statistic::where('own', 1)->get()->toArray();
        if ($request->has('year') && !$request->has('month')) {
            $cookingForHireStatistics = Cooking_for_hire_statistic::where('own', 1)->whereYear('created_at', $query['year'])->get()->toArray();
        }
